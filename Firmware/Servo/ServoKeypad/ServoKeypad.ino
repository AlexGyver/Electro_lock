...
boolean open_bat_low = 1;  // 1 - открыть дверь, если акум разряжен (просыпаемся каждые 10 мин и проверяем), 0 - нет
...
#define servoMax 180       // угол сервы для положения "замок закрыт", подбирается экспериментально
//-------------------------------НАСТРОЙКИ-----------------------------------
#define close_button 1     // 0 если используем захлопывающуюся щеколду, 1 если серво или привод замка (есть кнопка закрытия или концевик)
...
byte rowPins[ROWS] = {11, 10, 9, 8};     // Подключены строки (4 пина)
byte colPins[COLS] = {7, 6, 5};          // подключены столбцы (4 пина)
...
void open_door() {
  //--------ОТКРЫТЬ---------
  servo.attach(A3);                     // привязываем привод к порту A3
...
void loop() {
  ...
  if (millis() - pass_timer > 6000) { // если сработал таймер
    j = 0;
  }
  //-----------------ОТРАБОТКА ВВОДА ПАРОЛЯ-------------------
  ...
  if (set_access_flag) {
    ...
    while (1) {                          // бесконечный цикл
      ...
      if (millis() - pass_timer > 6000) { // если сработал таймер
        // сбросить ввод и выйти из цикла
        int_pass = EEPROM.readLong(0);     // вспоминаем пароль из памяти
        str_pass = String(int_pass, DEC);  // переводим в строчный тип
        pass_lenght = str_pass.length();   // получаем длину пароля
